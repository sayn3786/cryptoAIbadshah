...
out.due = Object.fromEntries([[0,2],[0,5],[0,7],[0,15],[4,2],[4,12],[4,32],[4,35],
  [8,7],[12,7],[16,15],[3,59],[13,33]].map(([h,m]) => [`${h}:${m}`, w.dueJobs(at(h,m))]));

const calls = [];
let script = [];                          // queued statuses per path
globalThis.fetch = async (url, opts) => {
  const path = new URL(url).pathname;
  calls.push({ path, method: opts.method, headers: opts.headers });
...
def test_schedule(result):
    ...
    assert due["12:7"] == ["manage"]                      # 12:05 stays on Vercel's cron
