...
live parameter. A change that acts on a discriminator is a separate, backtested,
human-approved strategy_version (v47 raised the R/R floor after thin_reward_to_risk
came out over-represented).
...
| GET | `/api/signals/timing-report` | public | When and on what timeframe the trades work, POOLED across every `strategy_version` (timeframe and time-of-day are structural, not a property of one rule-set): timeframe efficiency (win rate, expectancy, expectancy per day of capital at risk) and session/hour performance. Pure logic in `backend/signal_analytics.py`. Read-only; never tunes. `environment`, `limit`. |
| GET | `/api/paper/account` | public | Paper account — what auto-executing the closed signals would have made/lost at a fixed notional, net of taker fees. Pure preview in `backend/paper_account.py`: no keys, no orders, no exchange. Flags when the paper size is below Hyperliquid's ~$10 live minimum. `trade_size_usd`, `start_balance_usd`, `fee_bps`, `strategy_version`, `environment`, `limit`. |
